...
export default async ({{ client }}) => {{
  ...
  return {{
    'experimental.chat.messages.transform': async (_input, output) => {{
      ...
      if (firstUser.parts.some(p => p.text && p.text.includes('<PLUGIN_CONTEXT name="{name}">'))) return;
...
"""
        for tool in tool_defs:
            ...
            js_code += f"""      {{
        name: "{tool["name"]}",
        description: "Execute script from plugin {name}: {Path(tool["path"]).name}",
        execute: async (args) => {{
          const scriptArgs = typeof args === 'string' ? args : JSON.stringify(args);
          return await client.runShellCommand("{cmd} {tool["path"]} " + scriptArgs);
        }}
      }},
"""
