...
def test_install_codeagent_server_all_targets(tmp_path, home, calls):
    results = mcp_service.install_codeagent_server(
        str(tmp_path), targets=["claude", "codex"], allow_write=True
    )
    assert len(results) == 2
    assert all(r["action"] == "added" for r in results)
    assert all(r["name"] == "codeagent" for r in results)

    # Verify command contains --allow-write
    for _, _engine, name, kwargs in calls:
        assert name == "codeagent"
        assert kwargs["command"][-1] == "--allow-write"


def test_install_codeagent_server_read_only(tmp_path, home, calls):
    results = mcp_service.install_codeagent_server(
        str(tmp_path), targets=["claude"], allow_write=False
    )
    assert results[0]["action"] == "added"
    # Command should not have --allow-write
    for _, _engine, _name, kwargs in calls:
        assert "--allow-write" not in kwargs["command"]


def test_remove_codeagent_server(tmp_path, home, calls):
