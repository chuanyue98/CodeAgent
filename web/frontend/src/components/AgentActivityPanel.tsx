  ...
  return (
    <aside className="glass-card flex w-80 shrink-0 flex-col p-4">
      <div className="mb-3 flex items-center justify-between border-b border-slate-100 pb-3">
        <div>
          <p className="text-sm font-semibold text-slate-800">{t('agent.activity')}</p>
          <p className="text-[10px] text-slate-400">
            {t('agent.activitySubtitle')}
          </p>
        </div>
        <button
          aria-label={t('agent.closeActivity')}
          onClick={onClose}
      ...
      <div className="custom-scrollbar min-h-0 flex-1 space-y-2 overflow-y-auto">
        {activity.length === 0 && (
          <p className="text-xs italic text-slate-400">{t('agent.noActivity')}</p>
        )}
        {activity.map(event => (
          <details
            key={event.sequence}
