...
export default function GlobalTerminalDrawer() {
  ...
  const handleHandoff = async (targetEngineId: string) => {
    ...
    try {
      await convertAndLaunchSession(activeTab.sessionId, targetEngineId);
      closeTab(activeTab.id);
    } catch (err: any) {
      setHandoffError(err.message || String(err));
    } finally {
  ...
  if (!isDrawerOpen) {
    return (
      <div 
        className="fixed bottom-0 left-0 right-0 z-50 flex items-center justify-between border-t border-slate-200 bg-white/95 px-4 py-2 shadow-[0_-4px_10px_rgba(0,0,0,0.05)] backdrop-blur transition-transform hover:bg-slate-50 cursor-pointer"
        onClick={openDrawer}
  ...
  return (
    <div 
      className={`fixed bottom-0 left-0 right-0 z-50 flex flex-col bg-white shadow-[0_-10px_40px_rgba(0,0,0,0.1)] transition-all duration-300 ${isMaximized ? 'top-0' : 'h-[60vh] min-h-[300px]'}`}
