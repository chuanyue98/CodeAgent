...
Stand-ins for `claude`/`codex`/`gemini`/`opencode`, prepended onto `PATH` for
...
```
cp claude codex gemini opencode
```
...
2. `mcp remove <name>` — only for claude/codex (the only two engines
   `mcp_service.py` actually shells out to for removal; gemini/opencode
