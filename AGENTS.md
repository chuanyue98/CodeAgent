...
- ## 主线 2：【Web 端一键会话无缝接力】（高频刚需，落地极快）

  • 核心目标：把已经实现好的 ca switch 搬到 Web 界面上。
  • 实际场景：在网页自带的 Terminal 或 Sessions
  列表里聊着天，突然模型额度不够了，或者想换个模型试试，点击界面的「接力到其他引擎」按钮，后台自动
  转换会话格式并在新标签页中无缝继续聊，彻底不用记命令行参数。你看看这个怎么样感觉？
