...
def _merge_breakdown(
    breakdowns: dict[str, ModelBreakdown], entry: RawUsageEntry
) -> None:
...
def aggregate(entries: list[RawUsageEntry]) -> dict[str, Any]:
    ...
    def update_usage(usage_obj, entry: RawUsageEntry):
    ...
    for e in entries:
        ...
        update_usage(daily[d_key], e)
        _merge_breakdown(daily_bds[d_key], e)

        # Monthly
        m_key = (_month(e.timestamp), e.target)
        if m_key not in monthly:
            monthly[m_key] = MonthlyUsage(month=m_key[0], target=e.target)
        update_usage(monthly[m_key], e)
        _merge_breakdown(monthly_bds[m_key], e)
        ...
        _merge_breakdown(session_bds[s_key], e)
...
def _nest_subtasks(sessions: dict[tuple, SessionUsage]) -> list[SessionUsage]:
    ...
    dropping it would take its usage out of the list entirely.
    """
...
def _rolled_totals(su: SessionUsage) -> tuple[int, int, int, int]:
    """(input, output, cache write, cache read) including every subtask."""
    ...
    for child in su.subtasks:
        child_in, child_out, child_write, child_read = _rolled_totals(child)
        input_t += child_in
        output_t += child_out
...
def _session_to_dict(su: SessionUsage) -> dict:
    ...
    Token counts and ``lastActivity`` are rolled up over the session's
    subtasks, so a row reads as "what this piece of work used". ``own`` keeps
    ...
    """
    input_t, output_t, cache_write, cache_read = _rolled_totals(su)
